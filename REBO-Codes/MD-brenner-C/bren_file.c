...
#include "brenner.h"
/* stdlib.h is for abort, among others. */
#include <stdlib.h>
#include <unistd.h>
#include <string.h>
#include <getopt.h>
...
init_coord(BrennerMainInfo *info)
{
  ...
  for(i = firstIndex (toState (info)); i < np; ++i)
  {
  ...
  for(i = firstIndex (toState (info)); i < np; ++i)
  {
    ...
    sscanf(buf, "%d", &k);
    --k;
    assert(k >= firstIndex (toState (info)) && k < MAX_ATOMS);
    {
      double x, y, z;
      if(sscanf(buf, "%d %lf %lf %lf", &dummy, &x, &y, &z) != 4)
	{
...
#endif
      for (atm = firstIndex (toStateconst (info)); atm < info->num_atms; atm++)
      {
