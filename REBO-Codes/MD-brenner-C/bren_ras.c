...
read_atom_info(char *filename, BrennerMainInfo *info, double cube_init)
{
	...
	ForEachAtom {
		...
		int natom;
		double t;
		int itr = 0; /* make all atoms movable ????????? */
...
main(int argc, char **argv)
{
	...
	while ((i = (int) getopt(argc, argv, "?c:d:s:t:")) != -1)
	{
	  extern char *optarg;
	  switch(i)
	  {
	  case '?':
	    printf("useage: brennerrasmol [-s <#steps>] [-t <temperature(K)>] [-c <name of output file>] [-d <size of cube>] size input_file");
	    exit(-1);
	  case 'c':
