...
struct NeighborState {
  ...
  /* Neighbors_allocated is the number of integers we can fit into
     neighbor_list.  We may not have used that many; the number actually used
     has to be carried around separately. */
...
// NeighborState.
void sortNeighbors (struct NeighborState* ns,
                    int firstAtom, int pastLastAtom); 

// Do some consistency checks. 
void validate (const struct NeighborState *ns,
               int firstAtom, int pastLastAtom);
