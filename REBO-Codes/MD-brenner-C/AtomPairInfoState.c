...
#endif
    ;
  struct AtomPairInfoState *apis =
    (struct AtomPairInfoState *) xmalloc (sizeof (struct AtomPairInfoState));
  apis->ai_allocated = initial_allocation;
  apis->atoms_allocated = initial_allocation;
  apis->ai_start = (int *) xmalloc (sizeof (int) * initial_allocation);
  apis->ai_list =
    (struct AtomPairInfo *)
    xmalloc (sizeof (struct AtomPairInfo) * initial_allocation);
  return apis;
}
