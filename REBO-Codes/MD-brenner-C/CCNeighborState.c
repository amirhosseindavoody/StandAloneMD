...
struct NeighborState *newNeighborState () {
  ...
  struct NeighborState *ljns =
    ...
    xmalloc (sizeof (struct NeighborState) * initial_allocation);
  ljns->neighbors_allocated = initial_allocation;
  ljns->atoms_allocated = initial_allocation;
  ljns->neighbor_start = (int *) xmalloc (sizeof (int) * initial_allocation);
  ljns->neighbor_list = (int *) xmalloc (sizeof (int) * initial_allocation);
  return ljns;
}
...
void sortNeighbors (struct NeighborState *const ns,
                    const int firstAtom,
                    const int pastLastAtom) {
  int i;
  assert (sizeof(int) == sizeof (const void *));
  for (i = firstAtom; i < pastLastAtom; i++) {
...
void validate (const struct NeighborState *const ns,
               const int firstAtom, const int pastLastAtom) {
  int i;
  assert (pastLastAtom <= ns->atoms_allocated);
  for (i = firstAtom; i < pastLastAtom; i++) {
    assert (0 <= ns->neighbor_start [i]);
    assert (ns->neighbor_start [i] < ns->neighbors_allocated);
    if (i < ns->atoms_allocated - 1) {
      int j;
      assert (ns->neighbor_start [i] <= ns->neighbor_start [i+1]);
      for (j = ns->neighbor_start [i]; j < ns->neighbor_start [i+1]; j++) {
        assert (0 <= ns->neighbor_list [j]);
        assert (ns->neighbor_list[j] < ns->neighbors_allocated - 1);
      }
    }
  }
}
...
void deallocateNeighborState (struct NeighborState *ljns) {
  free (ljns->neighbor_start);
  free (ljns->neighbor_list);
  free (ljns);
}
