...
deallocate_caguts () {
  if (apis) {
    deallocateAtomPairInfoState (apis);
    apis = 0;
  }
}
...
#ifdef DEBUGPRINT
        if (spy == i && 715 == j) {
          printf ("For %d and %d, rc is %f, rsq is %f, rmax is %f, "
                  "ddtab is %f\n",
                  i, j, rc, rsq, rmax[ki][kj], ddtab[ki][kj]);
          printf ("For %d and %d, rp is %f, thisPair->cor.x is %f\n", i, j, rp,
                  thisPair->cor.x);
        }
#endif
...
#ifndef NDEBUG
        if (spy == i && 715 == j) {
          printf ("For %d and %d, rt is %f, it is %d, drtable[%d][%d][it] is %f, drtable[it+1] is %f\n",
                  i, j, 
                  rt, it, ki, kj, drtable[ki][kj][it],
                  drtable[ki][kj][it+1]);
          printf ("For %d and %d, rp is %f, thisPair->cor.x is %f\n", i, j, rp,
                  thisPair->cor.x);
        }
