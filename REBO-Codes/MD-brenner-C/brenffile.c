...
init_coord(BrennerMainInfo *info)
{
  ...
  sscanf(buf, "%d", &np);
  fgets(buf, sizeof(buf), fp);
  if(sscanf(buf, "%lf %lf", &arg1, &arg2) != 2)
  {
    fprintf(stderr, "Error parsing times %s\n", buf);
    exit(-1);
  }
  info->starttime = arg1;
  info->timestep = arg2;
  ...
  {
    if(sscanf(buf, "%lf %lf %lf", &arg1, &arg2, &arg3) != 3)
    {
      fprintf(stderr, "Error parsing cube %s\n", buf);
      exit(-1);
    }
    info->cube[0] = arg1;
    info->cube[1] = arg2;
  ...
  for(i = 0; i < np; ++i)
  {
    ...
    atm_num[k].ktype = kt[natom];
    ++info->noa[kt[natom]];

    atm_num[k].number = k;
    atm_num[k].type = natom;
    atm_num[k].mass = xmass[kt[natom]];
  ...
  for(i = 0; i < np; ++i)
  {
    ...
    --k;
    assert(k >= 0 && k < MAX_ATOMS);
    if(sscanf(buf, "%d %lf %lf %lf", &dummy, &arg1, &arg2, &arg3) != 4)
    {
      fprintf(stderr, "Error parsing velocity %d, %s\n", k, buf);
      exit(-1);
    }
    atm_num[k].velocity.x = arg1;
    atm_num[k].velocity.y = arg2;
    atm_num[k].velocity.z = arg3;
#if 0
  ...
  for(i = 0; i < np; ++i)
  {
    ...
    atm_num[k].accel.x = arg1;
    atm_num[k].accel.y = arg2;
    atm_num[k].accel.z = arg3;
  }
  for(i = 0; i < np; ++i)
  {
    if(!fgets(buf, sizeof(buf), fp))
    ...
    atm_num[k].dx3dt3.x = arg1;
    atm_num[k].dx3dt3.y = arg2;
    atm_num[k].dx3dt3.z = arg3;
  }
  printf("read %d atoms\n", info->num_atms);
  fclose(fp);
}
...
append_file(FILE *fp, const BrennerMainInfo *info, double TTIME)
{
      ...
      for (atm = 0; atm < info->num_atms; atm++)
      {
		   ...
		   atm+1, kt2[info->atm_num[atm].ktype],
